...
static my_bool show_plugins(THD *thd, plugin_ref plugin, void *arg)
{
  ...
  version_len= my_snprintf(version, sizeof(version), "%d.%d",
    ...
    table->field[0]->store(name, name_len, system_charset_info); 
    table->field[1]->store(version, version_len, system_charset_info);
    table->field[2]->store(3,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
      return 1;                                       
  ...
  table->field[0]->store(name, name_len, system_charset_info); 
  table->field[1]->store(plugin_ref_to_int(plugin)->locks_total, UNSIGNED);
  table->field[2]->store(3,UNSIGNED); 
  if (schema_table_store_record(thd, table))        
      return 1;                                       
...
int fill_linux_info(THD *thd, TABLE_LIST *tables)
{
  ...
  if (have_ubuf)
  {
    table->field[0]->store("Uname_sysname", sizeof("Uname_sysname")-1 , system_charset_info); 
    table->field[1]->store(ubuf.sysname, strlen(ubuf.sysname), cs);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;                                       
    table->field[0]->store("Uname_release", sizeof("Uname_release")-1 , system_charset_info); 
    table->field[1]->store(ubuf.release, strlen(ubuf.release), cs);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;                                       
    table->field[0]->store("Uname_version", sizeof("Uname_version")-1 , system_charset_info); 
    table->field[1]->store(ubuf.version, strlen(ubuf.version), cs);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;                                       
    table->field[0]->store("Uname_machine", sizeof("Uname_machine")-1 , system_charset_info); 
    table->field[1]->store(ubuf.machine, strlen(ubuf.machine), cs);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;                                       
  ...
  if (have_distribution)
    table->field[0]->store("Uname_distribution", sizeof("Uname_distribution")-1 , system_charset_info); 
    table->field[1]->store(distribution, strlen(distribution), cs);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;     
#endif
...
#ifdef MY_ATOMIC_OK
    table->field[0]->store("Cpu_count", sizeof("Cpu_count")-1 , system_charset_info); 
    table->field[1]->store(my_getncpus(), UNSIGNED);
    table->field[2]->store(4,UNSIGNED); 
    if (schema_table_store_record(thd, table))        
        return 1;    

#endif
  table->field[0]->store("Mem_total", sizeof("Mem_total")-1 , system_charset_info); 
  table->field[1]->store(my_getphysmem(), UNSIGNED);
  table->field[2]->store(4,UNSIGNED); 
  if (schema_table_store_record(thd, table))        
        return 1;    
  table->field[0]->store("Now", sizeof("Now")-1 , system_charset_info); 
  table->field[1]->store(thd->query_start(), UNSIGNED);
  table->field[2]->store(4,UNSIGNED); 
  if (schema_table_store_record(thd, table))        
        return 1;    
